...
const REMOTE_INPUT_JSON_FILE = "news.remote.json";
...
async function main() {
  ...
  const remotePayload = await readJson(REMOTE_INPUT_JSON_FILE, null);

  const snapshotItems = Array.isArray(snapshotPayload?.items) ? snapshotPayload.items : [];
  const remoteItems = Array.isArray(remotePayload?.items) ? remotePayload.items : [];
  const useRemote = remoteItems.length > 0;

  if (!useRemote && (!snapshotPayload || !Array.isArray(snapshotPayload.items))) {
    throw new Error(`${INPUT_JSON_FILE} invalid or missing`);
  }

  const activePayload = useRemote ? remotePayload : snapshotPayload;
  const activeItems = useRemote ? remoteItems : snapshotItems;
  const inputSource = useRemote ? "remote" : "snapshot";
  ...
  const payload = {
    ...
    snapshotAt: String(activePayload?.snapshotAt || ""),
    remoteAgentAt: String(remotePayload?.agentAt || remotePayload?.remoteAgentAt || ""),
    inputSource,
  ...
  const jsLines = [
    ...
    `window.__NEWS_AGENT_META__ = ${JSON.stringify({
      ...
      remoteAgentAt: payload.remoteAgentAt,
  ...
  console.log(`[agent] wrote ${OUTPUT_JS_FILE} and ${OUTPUT_JSON_FILE} with ${finalItems.length} items (candidates=${mergedRows.length}, input=${inputSource})`);
}
