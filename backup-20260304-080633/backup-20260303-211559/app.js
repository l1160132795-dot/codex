...
function toChineseHeadline(raw, category = "Market") {
  ...
  if (localized !== clean) return summarizeHeadline(localized);
  if (latinRatio(localized) <= 0.68) return summarizeHeadline(localized);
  const fallback = buildChineseHeadlineFallback(localized, category);
  if (fallback) return summarizeHeadline(fallback);
  return summarizeHeadline(localized || clean);
}
...
function readSnapshotNewsPayload() {
  ...
  return { rows, snapshotAtRaw };
}

function buildNewsFromSnapshot() {
  const { rows, snapshotAtRaw } = readSnapshotNewsPayload();
  ...
  const recent = normalized.filter((row) => Number.isFinite(row.time) && row.time >= now - maxAgeMs && row.time <= now + 10 * 60 * 1000);
...
function enrichNewsItems(items) {
  ...
  return items.map((item) => {
    ...
    const impact = (categoryWeight[item.category] || 2.2) + Math.abs(tone) * 1.2 + recency + engagement + major;
    return {
      ...item,
      tone,
      impact,
      brief: toChineseBrief(item.title, item.category),
      headlineZh: toChineseHeadline(item.title, item.category),
      size: "sm"
    };
  });
}
...
async function fetchHotAltBoard() {
  try {
    return await fetchHotAltBoardCoinGecko();
  } catch (e1) {
    try {
      return await fetchHotAltBoardBinance();
    } catch (e2) {
      throw new Error(`${e1.message}; ${e2.message}`);
    }
  }
...
function renderAltCluster(board, contextMap) {
  ...
  const cy = stageRect.height * 0.52;
  const minDim = Math.min(stageRect.width, stageRect.height);

  let html = "";
  const centerSize = clamp(126 + (Math.abs(leader.changePct || 0) / maxAbs) * 42, 126, 176);
  html += `
  ...
  others.forEach((coin, idx) => {
    ...
    const ring = idx < 8 ? minDim * 0.24 : idx < 18 ? minDim * 0.34 : minDim * 0.43;
    const x = cx + Math.cos(angle) * ring;
    const y = cy + Math.sin(angle) * ring;
    const size = clamp(64 + (Math.abs(coin.changePct || 0) / maxAbs) * 38, 60, 118);
    html += `
...
async function refresh() {
  ...
  const fearGreedRes = results[1].status === "fulfilled" ? results[1].value : null;
  const globalRes = results[2].status === "fulfilled" ? results[2].value : null;
  const newsRes = results[3].status === "fulfilled"
    ? results[3].value
    : { items: [], sourceStats: {}, failed: -1, totalSources: 0, stale: false, staleMinutes: 0, snapshot: false, snapshotMinutes: null };
  const altBoardRes = results[4].status === "fulfilled" ? results[4].value : [];
  const officialRes = results[5].status === "fulfilled" ? results[5].value : FALLBACK_EVENTS;
  ...
  if (!tickerRes) {
    ...
    els.newsMeta.innerHTML = `<span class="error-tip">核心行情获取失败：${escapeHtml(results[0].reason?.message || "Unknown error")}</span>`;
  ...
  if (newsRes.snapshot) {
    if (!headlines.length) {
      els.newsMeta.innerHTML = "<span class='error-tip'>本地新闻快照为空，请先更新 news.snapshot.js。</span>";
      ...
      els.newsMeta.textContent = `新闻快照模式：${sourceSummary} 个来源，快照更新时间约 ${newsRes.snapshotMinutes} 分钟前（前端不直连外部新闻接口）。`;
    } else {
